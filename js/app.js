...
else if(OnDisplayNow==="green"){loadGraySneakers();pictureList[movementTracker].style.display='block';
}
else{loadBlueSneakers();pictureList[movementTracker].style.display='block';
}
...
GraySneakers={                                                     
    ...
    images:['images/NikeAirForce.jpg' , 'images/NikeAirForceGray.jpg'],
    price:"200",
...
let loadGraySneakers=()=>{                                        
   
    deleteCurrentImages("product-image");
    for(let i=GraySneakers.images.length; i>=0;i--){
    let newImage=document.createElement("img");
    newImage.src=GraySneakers.images[i];
    newImage.className=GraySneakers.className;
    imageParentDiv.insertBefore(newImage,pictureList[0])
    }
    pictureList[movementTracker].style.display='block';
    productName.innerHTML = GraySneakers.banner;
    productPrice.innerHTML = GraySneakers.price;
    colorDescription.innerHTML = GraySneakers.productInfo;
    OnDisplayNow = "green";
}

let loadBlueSneakers=()=>{                                     
    
    deleteCurrentImages("product-image");
    for(let i=BlueSneakers.images.length-1; i>=0;i--){
    let newImage=document.createElement("img");
    newImage.src=BlueSneakers.images[i];
    newImage.className=GraySneakers.className;
    console.log(newImage);
    imageParentDiv.insertBefore(newImage,pictureList[0])
    }
    pictureList[movementTracker].style.display='block';
    productName.innerHTML = BlueSneakers.banner;
    productPrice.innerHTML = BlueSneakers.price;
    colorDescription.innerHTML = BlueSneakers.productInfo;
    OnDisplayNow = "blue";
}

colorBtn1.addEventListener("click",(e)=>{  loadGraySneakers();}); 
colorBtn2.addEventListener("click",(e)=>{ loadBlueSneakers();});
...
function orderHandler(){
    ...
    function imageLink(){             
        ...
        if(OnDisplayNow==='green'){
            theUrl=GraySneakers.images[1];
        }
        else{
            theUrl=BlueSneakers.images[1];
        }
        return theUrl;
    }
